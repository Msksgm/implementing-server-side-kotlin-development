...
class ArticleRepositoryImplTest {
        ...
        fun `正常系-DB に作成済記事がある場合、全ての作成済記事の一覧が戻り値`() {
            ...
            val expected = listOf(
                ...
                CreatedArticle.newWithoutValidation(
                    ...
                    title = Title.newWithoutValidation("dummy-slug-02"),
            ...
            when (actual) {
                ...
                is Either.Right -> {
                    ...
                    createdArticleList.forEach { actualArticle ->
                        ...
                        assertThat(actualArticle.title).isEqualTo(actualArticle.title)
                        assertThat(actualArticle.body).isEqualTo(actualArticle.body)
                        assertThat(actualArticle.description).isEqualTo(actualArticle.description)
                    }
                }
            }
        }
    ...
    class Delete {
        ...
        @ExpectedDataSet(
            value = ["datasets/yml/then/delete-articles.yml"],
            orderBy = ["id"],
        )
        // NOTE: @ExportDataSetはgivenの@DataSetが変更された時用に残しておく
        // @ExportDataSet(
